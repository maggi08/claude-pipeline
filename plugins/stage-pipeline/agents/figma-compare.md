...
По каждому `AC-*`, переданному в промпте, — отдельная строка вердикта, и **PASS несёт замеренное значение так же, как FAIL**: «`AC-2.4` — PASS, радиус 12px (`Button.vue:34` ↔ node 12:445)». Голый `PASS` неотличим от «не посмотрел» и оркестратором засчитан не будет; критерий, до которого ты не добрался, помечай `не проверено` честно — это не хуже FAIL, это единственное, что можно исправить.
