...
8. **MCP-зависимости — проверить.** Пайплайну для `figma-compare`/`figma-spec` нужен Figma MCP, для `devtools-verify` — Chrome DevTools MCP, для сверки API библиотек (`docs-lookup`, pro-review, deps-audit) — `context7`. Все три приезжают в `.mcp.json` плагина, поэтому подключать руками обычно не нужно — достаточно `claude mcp list` (ждём «✓ Connected») и разрешить серверы плагина, если Claude Code спрашивает подтверждение.
   ...
   - `context7` без авторизации не подключается (401, needs-auth): `export CONTEXT7_API_KEY=ctx7sk-…` (бесплатный ключ, context7.com/dashboard) в профиле шелла или OAuth через `/mcp`; подробности — `/pipeline-doctor`, п.1.
...
2. Отчитайся по MCP (шаг 8): что подключено и живо, что требует действия пользователя (запустить Figma desktop, одобрить серверы плагина, `CONTEXT7_API_KEY` для context7).
