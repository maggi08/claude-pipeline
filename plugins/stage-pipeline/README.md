...
- **git-guard** — на ветке задачи в обычном режиме `git add`/`git commit` агента отклоняются при любом статусе этапа: агент выдаёт готовое сообщение коммита, коммитишь ты сам. В force-режиме (незакрытый блок «Force-прогон» в STAGES.md — без «завершён» в заголовке) агент делает `git add` + `git commit` сам, без вопросов, — но коммит проходит через `floor-guard` и отклоняется, если в дифе заглушён чекер, ослаблен тест или оставлена заглушка. Проверяется то, что войдёт в коммит: индекс, пути из `git add` той же команды, все отслеживаемые при `-a`, всё дерево — только при `git add -A` без путей.
