...
- **Секретов в плагине нет.** В `.mcp.json` два локальных сервера (`127.0.0.1:3845` для Figma, `npx chrome-devtools-mcp`) и один внешний — `https://mcp.context7.com`.
- **Context7 — единственное, что уходит с машины разработчика наружу.** Сервер хостит Upstash; в запросах — имя библиотеки и вопрос по её API, который формулирует агент (`docs-lookup`, `pro-review`, `deps-audit`, `/root-cause`). Код репозитория целиком не отправляется, но в формулировке вопроса могут оказаться имена внутренних модулей и фрагменты использования API. Если политика команды это запрещает — отключите сервер у себя: `/mcp` → `plugin:stage-pipeline:context7` → Disable (или `"disabledMcpjsonServers": ["context7"]` в `.claude/settings.json` продуктового репо — отключит у всей команды). Без него чекеры помечают такие пункты «не сверено с доками» и работают дальше.
