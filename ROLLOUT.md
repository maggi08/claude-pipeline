...
- **git-guard** — в обычном режиме на ветке задачи, пока этап в работе, `git add`/`git commit` агента отклоняются: агент выдаёт готовый текст коммита, коммитит разработчик в своём терминале — даже если попросить агента. В force-режиме (`/stage-force`) агент коммитит сам, но коммит проходит через `floor-guard` и отклоняется, если в дифе заглушён чекер, ослаблен тест или оставлена заглушка.
